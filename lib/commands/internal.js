...
function InternalCommand(name, man, hasText, argNames, requiresPost, callback) {
	Command.call(this, name, man, hasText, argNames, {}, requiresPost);
...
CommandBuilder.prototype.create = function(callback) {
	debug('Creating internal command with', this.name, this.manual, this.hasText, this.argNames, this.requiresPost);
	commands[this.name] = new InternalCommand(this.name, this.manual, this.hasText, this.argNames, this.requiresPost, callback);
};

CommandBuilder('user.create')
	.man("Creates a new YoubNub user.")
...
CommandBuilder('user.view')
	.man("Views user info.")
	.arg('id', 'The users’id. Can be passed via text as well', null)
	.create(function(text, args, req, res, next, commands) {
		var id = args.id || text;
		if(!id) {
