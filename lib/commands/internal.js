...
CommandBuilder('user.edit')
	...
	.create(function(text, args, req, res, next, commands) {
		var posted = req.body._id;
		if(posted) {
			var update = {};
			['userId', 'name', 'aliases', 'envs'].forEach(function(type) {
				if(type in args) {
			...
			req.collections.users.updateOne({_id: new ObjectID(posted)}, {$set: update}, {upsert: true}, function(err, result) {
				if(err) {
					return next(err);
				}
				if(!result.matchedCount) {
					return res.render('message', {status: "Invalid user "+update.userId, message: "User could not be updated."});
				}
				res.redirect('/'+posted);
			});
		} else {
